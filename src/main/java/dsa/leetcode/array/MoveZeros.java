...
public class MoveZeros {
  ...
  public int[] moveZeros(int[] nums) {
    ...
    while (zeroIndex < nums.length && nonZeroIndex < nums.length & zeroIndex < nonZeroIndex) {
      ...
      if (nonZeroIndex < zeroIndex) nonZeroIndex = zeroIndex + 1;
      while (nonZeroIndex < nums.length && nums[nonZeroIndex] == 0) nonZeroIndex++;
      if (!(zeroIndex < nums.length && nonZeroIndex < nums.length)) return nums;
